...
> **Note on branches.** The 2.0.0 rewrite lives on the `query_v3` branch
> and is released as the `v2.0.0` tag. The legacy 1.x adapter remains on
> `main` during the migration window, so the two coexist; `query_v3` is
> **not** being merged into `main` for now. Branch from and target
> `query_v3` for all new work. See
> [`TODO.md`](https://hms-dbmi.github.io/pic-sure-r-adapter-hpds/TODO.md)
> for the eventual `main` cutover.
