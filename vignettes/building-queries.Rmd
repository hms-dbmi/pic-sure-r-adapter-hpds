...
The same applies to `buildClauseGroup(root = picsure::GroupOperator$AND)` and `runQuery(type = picsure::QueryType$COUNT)`. Strings remain valid for backwards compatibility; the enum members are new in this release.
...
Both `type` (clause type) and `root` (group operator) accept
