...
| `bdc.searchPicsure(session, "sex")` | `search(session, "sex")` |
| `bdc.getStudies(session)` | (removed — not yet re-exposed) |
| `newQuery(session)` | no direct replacement — use `createClause()` + `buildClauseGroup()` |
| `addClause(q, path, "FILTER", min = 18)` | `createClause(path, "FILTER", min = 18)` |
| `addClause(q, path, "SELECT")` | `createClause(path, "SELECT")` |
| `deleteClause(q, path)` | (removed — rebuild the tree) |
| `showQuery(q)` | (removed — queries are opaque Python handles) |
| `runQuery(q, "COUNT")` | `runQuery(session, query, type = "count")` |
...
query <- picsure::buildClauseGroup(list(age_filter), root = "AND")
...
query <- picsure::buildClauseGroup(list(sex_filter, bmi_select), root = "AND")
...
query   <- picsure::buildClauseGroup(list(sex_filter, lung), root = "AND")
```
