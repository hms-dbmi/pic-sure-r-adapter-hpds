...
test_that("exportCSV() forwards query and path, returns path invisibly", {
  testthat::local_mocked_bindings(picsure_py = fake_picsure_py())
  bdc <- picsure::connect(platform = "Demo", token = "tok")
  q <- list(kind = "group", clauses = list(), root = "AND")
  tmp <- tempfile(fileext = ".csv")

  result <- withVisible(picsure::exportCSV(bdc, q, tmp))

  expect_false(result$visible)
  expect_equal(result$value, tmp)
  expect_true(file.exists(tmp))
  expect_identical(bdc$.calls$exportCSV[[1]]$query, q)
  expect_equal(bdc$.calls$exportCSV[[1]]$path, tmp)
})

test_that("exportTSV() forwards query and path, returns path invisibly", {
  testthat::local_mocked_bindings(picsure_py = fake_picsure_py())
  bdc <- picsure::connect(platform = "Demo", token = "tok")
  q <- list(kind = "group", clauses = list(), root = "AND")
  tmp <- tempfile(fileext = ".tsv")

  result <- withVisible(picsure::exportTSV(bdc, q, tmp))

  expect_false(result$visible)
  expect_equal(result$value, tmp)
  expect_true(file.exists(tmp))
  expect_identical(bdc$.calls$exportTSV[[1]]$query, q)
...
test_that("exportCSV() errors on missing path", {
  ...
  bdc <- picsure::connect(platform = "Demo", token = "tok")
  q <- list(kind = "group", clauses = list(), root = "AND")
  expect_error(picsure::exportCSV(bdc, q), "path")
})

test_that("exportTSV() errors on missing query", {
  testthat::local_mocked_bindings(picsure_py = fake_picsure_py())
  bdc <- picsure::connect(platform = "Demo", token = "tok")
  tmp <- tempfile(fileext = ".tsv")
  expect_error(picsure::exportTSV(bdc, path = tmp), "query")
})
...
test_that("exportCSV() re-raises Python exceptions as picsureError", {
  ...
  testthat::local_mocked_bindings(picsure_py = fake)
  bdc <- picsure::connect(platform = "Demo", token = "tok")
  bdc$exportCSV <- function(query, path, ...) {
    stop(structure(
  ...
  err <- tryCatch(
    picsure::exportCSV(bdc, list(kind = "group"), tempfile()),
    error = function(e) e
  )
  expect_s3_class(err, "picsureError")
