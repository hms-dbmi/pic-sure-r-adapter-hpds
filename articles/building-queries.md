...
Genomic filters narrow a query to participants who carry variants
matching specific annotation values. They are available on authorized
platforms (BDC authorized, NHANES authorized) and are AND-combined with
...
Pass a list of genomic filters to `buildQuery(genomicFilters = ...)` -
they are AND-combined. Genomic-only queries (no phenotypic filter) are
allowed:
